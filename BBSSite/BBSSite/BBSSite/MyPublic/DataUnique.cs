...
namespace BBSSite.MyPublic
{
    ...
    /// <summary>
    /// 自定义验证类特性，AttributeUsage表示DataUnique
    /// </summary>
    ...
    public class DataUnique : ValidationAttribute
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            if (value != null)
            {
                ...
                // 如果不为空，则表示需要制定其他条件查询，Key为属性名称，例如ID
                if(string.IsNullOrEmpty(Key) == false)
                {
                    // 通过反射获取出条件属性的值
                    KeyID = Convert.ToInt32(validationContext.ObjectInstance.GetType()
                        .GetProperty(Key).GetValue(validationContext.ObjectInstance));
                }
                // 通过传入表枚举值调用简单工程模式类的InitDataUnique方法
                // 并返回已实现了ICheckUnique接口的各实现类
